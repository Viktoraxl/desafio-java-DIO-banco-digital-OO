...
Scanner scan = new Scanner(System.in);
	...
	public String sacar() {
		...
		if (novoSaldo <= 0) {
			...
			System.out.println("Qual valor deseja sacar na sua Conta Corrente? ");
			double saque = scan.nextDouble();
			if (saque <= novoSaldo) {
				novoSaldo -= saque;
                setSaldo(novoSaldo); // Atualizando o saldo
                return "Saque efetuado com sucesso Sr(a) "+getCliente().getNome()+".\n"
                		+ "O saldo atualizado da sua Conta Corrente é " + novoSaldo;
            } else {
	...
	public String depositar() {
		double novoSaldo = getSaldo();
		System.out.println("Qual valor deseja depositar Conta Corrente? ");
	...
	public String transferir(Contas contaOrigem, Contas contaDestino) {
		...
		System.out.println("Ola Sr(a) "+contaOrigem.getCliente().getNome()+ ", digite o valor da transferencia da sua Conta"
				+ " Corrente para a conta do Sr(a) :"+contaDestino.getCliente().getNome());
	...
	public void extrato() {
		System.out.println("Saldo atual Conta Corrente: "+getSaldo());
	}	
}
